    ...
    def test_init(self):
        ...
        with runner.isolated_filesystem():
            ...
            self.assertTrue(
                ...
                "yaclog init did not create a file",
            )
            self.assertIn(
                location,
                result.output,
                "yaclog init did not echo the file's correct location",
            )
            ...
            self.assertTrue(
                os.path.exists(os.path.abspath(location)),
                "file no longer exists after overwrite",
            )
            self.assertIn(
                location,
                result.output,
...
class TestShow(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.log.add_version(name="Version 2.0.0").add_entry(
            "- entry number 2", "Added"
        )
